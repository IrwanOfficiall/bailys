...
async function processLocationThumbnail(input, options = {}) {
    ...
    if (typeof input === "string" && (input.startsWith("http://") || input.startsWith("https://"))) {
        ...
        throw new Error("thumbnail must be URL or Buffer");
    }
    const resized = await sharp(buffer).resize(maxWidth, maxHeight, { fit: "inside", withoutEnlargement: true }).jpeg({ quality }).toBuffer();
    return resized;
}
...
const generateWAMessageContent = async (message, options) => {
    ...
    if (hasNonNullishProperty(message, "location")) {
        ...
        if (locationData.jpegThumbnail) {
            try {
                locationData.jpegThumbnail = await processLocationThumbnail(locationData.jpegThumbnail, {
                    maxWidth: 300,
                    maxHeight: 300,
                    quality: 80
                });
            } catch (err) {
                options.logger?.warn("thumbnail process failed", err);
            }
        }
        buttonsMessage.locationMessage = locationData;
